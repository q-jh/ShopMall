...
<script>
  ...
  export default {
    ...
    methods:{
      //向服务器发请求获取search参数（服务器根据不同返回不同的数据进行展示）
      getData(){
        this.$store.dispatch('getSearchList',this.searchParams)
      },
      removeCategoryName(){
        //带给服务器参数是可有可无的，如果属性值为空的字符串，还是会把相应的字段发给服务器
        //但是你把相应的字段设置为undefined，这个字段就不会带给服务器，提高了性能
        ...
        this.getData()
        //地址栏也要改,进行路由跳转，
        //严谨：本意是删除query，如果路径出现params不应该删除，路由跳转的时候应该带着
        if(this.$route.params){   //if永远成立，this.$route.params是一直都存在的
      ...
      attrInfo(attr,attrValue){
        //属性ID:属性值:属性名
        let props = `${attr.attrId}:${attrValue}:${attr.attrName}`//作用是连起来，属性ID:属性值:属性名
        //数组去重
        if(this.searchParams.props.indexOf(props) == -1){  //indexOf如果没有查找到你想要的结果，就会返回-1，表示没有查找到,返回-1和-1相等了，if为真判断成立
      ...
      removeAttr(index){
        //再次整理参数
        this.searchParams.props.splice(index,1) //splice() 方法用于添加或删除数组中的元素，并返回新数组
        //再次发起请求
        this.getData()
      },
      //排序的函数
      changeOrder(flag){
        //flag形参是用户点击的时候传送来的，代表的是综合还是价格
    ...
    computed:{
      //mapGetters里面的写法传递的是数据，因为getters计算是没有划分模块的【home，search之类的】
      ...mapGetters([
        'goodsList',
      ]),
      isOne(){
    ...
    watch:{
      ...
      $route(newValue,oldValue){
        ...
        //再次发起请求
        this.getData()
        //每一次请求完毕，需要把1，2，3级分类的id置空，让它接收下一次相应的1，2，3id
        //关键字和分类名字不用清理，因为每一次路由发生变化，都会给他赋予新的数据
        this.searchParams.category1Id = undefined;
        this.searchParams.category2Id = undefined;
        this.searchParams.category3Id = undefined;
      }
